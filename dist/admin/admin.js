(() => {
  ...
  const { readSession, friendlyError, compressImage, authRequest } = client;
  let session = readSession();
  let postSaving = false;
  let postImageLoading = false;
  ...
  function saveSession(value) { session = client.saveSession(value); }
  ...
  async function api(table, query = '', options = {}) {
    ...
    return client.request(table, query, { ...options, authenticated: true });
  }
  async function refreshSessionIfNeeded() { session = await client.getSession(); }
  ...
  async function checkSetup() {
    try {
      ...
      if (!admins?.length) throw Object.assign(new Error('CMS administrator access required.'), { status: 403 });
      setupNotice.hidden = true;
      if (!document.querySelector('#media-view').hidden) loadMedia();
      if (!document.querySelector('#blog-view').hidden) loadPosts();
    } catch (error) {
      setupNotice.hidden = false;
      setupNotice.querySelector('p').textContent = friendlyError(error);
  ...
  document.querySelector('#sign-out').addEventListener('click', async () => {
    try { if (session?.access_token) await authRequest('logout', {}, (await client.getSession()).access_token); } catch {}
    saveSession(null);
    showLogin();
    loginForm.reset();
  });
  ...
  document.querySelector('#media-upload').addEventListener('change', async event => {
    ...
    try {
      for (const file of files) {
        ...
        await api('cms_media', '', { method:'POST', body:JSON.stringify({ name:file.name, alt_text:file.name.replace(/\.[^.]+$/, '').replace(/[-_]+/g, ' '), data_url:image.dataUrl, width:image.width, height:image.height, created_by:session.user?.id }), headers:{ Prefer:'return=representation' } });
      }
  ...
  async function loadMedia() {
    ...
    try {
      ...
      grid.querySelectorAll('[data-delete-media]').forEach(button => button.addEventListener('click', async () => {
        ...
        try { const deleted = await api('cms_media', `?id=eq.${button.dataset.deleteMedia}`, { method:'DELETE', headers:{ Prefer:'return=representation' } }); if (!deleted?.length) throw new Error('The picture was not deleted. Refresh the media library and try again.'); loadMedia(); }
  ...
  function updatePostPreview() {
    ...
    postForm.querySelector('button[type="submit"]').disabled = postSaving || postImageLoading || !data.get('title')?.trim() || !data.get('excerpt')?.trim() || !data.get('body')?.trim();
  }
  ...
  postForm.elements.image.addEventListener('change', async event => {
    ...
    if (!file) return;
    postImageLoading = true; updatePostPreview();
    try {
      const image = await compressImage(file);
    ...
    } catch (error) { document.querySelector('#post-status').textContent = friendlyError(error); }
    finally { postImageLoading = false; updatePostPreview(); }
  });
  ...
  postForm.addEventListener('submit', async event => {
    ...
    if (postSaving || postImageLoading) return;
    const id = data.get('id');
    const existing = postsCache.find(post => post.id === id);
    const state = data.get('status');
    const payload = {
      slug:existing?.slug || slugify(data.get('title')) || `article-${crypto.randomUUID().slice(0, 8)}`, title:data.get('title').trim(), subheading:data.get('subheading').trim(), excerpt:data.get('excerpt').trim(),
      ...
      status:state, published_at:state === 'published' ? (existing?.published_at || new Date().toISOString()) : null, updated_at:new Date().toISOString(), ...(!id ? { created_by:session.user?.id } : {})
    };
    postSaving = true;
    submit.disabled = true; submit.textContent = 'Saving…'; status.textContent = '';
    try {
      let saved;
      if (id) saved = await api('cms_posts', `?id=eq.${encodeURIComponent(id)}`, { method:'PATCH', body:JSON.stringify(payload), headers:{ Prefer:'return=representation' } });
      else saved = await api('cms_posts', '', { method:'POST', body:JSON.stringify(payload), headers:{ Prefer:'return=representation' } });
      if (!saved?.length) throw new Error('The database did not confirm this article. Refresh the article list and try again.');
      status.textContent = state === 'published' ? 'Article published. It is now visible on the Blog page.' : 'Draft saved.';
      clearPostForm();
      loadPosts();
    } catch (error) { status.textContent = friendlyError(error); }
    finally { postSaving = false; submit.textContent = 'Save article'; updatePostPreview(); }
  });
  ...
  document.querySelector('#clear-post').addEventListener('click', () => { if (!postSaving && !postImageLoading) clearPostForm(); });
  document.querySelector('#remove-post-image').addEventListener('click', () => {
    if (postSaving || postImageLoading) return;
    currentPostImage = ''; postForm.elements.image.value = '';
    document.querySelector('#post-image-name').textContent = 'No picture selected'; updatePostPreview();
  });
